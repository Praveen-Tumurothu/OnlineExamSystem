...
public class StudentController {
	...
	public ResponseEntity<List> studentLogin(@RequestBody Student s) throws Throwable {

		List existence = studentservice.loginStudent(s.getUsername(), s.getPassword());
		return new ResponseEntity<List>(existence, HttpStatus.OK);
	...
	public ResponseEntity<Student> deEnrollStudent(@PathVariable int studentId, @PathVariable int courseId,
		...
		return new ResponseEntity<Student>(
				adminStudentManagement.deEnRollstudent(studentId, courseId, batchName, enrollmentId), HttpStatus.OK);

	}
	
	@GetMapping("/{studentId}")
	public ResponseEntity<Student> getStudent(@PathVariable int studentId) throws Exception{
		return new ResponseEntity<Student>(studentservice.getStudent(studentId), HttpStatus.OK);
	}
	
	@GetMapping("/getExamsByStudentID/{studentId}")
